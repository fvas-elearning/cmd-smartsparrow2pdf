...
try {
    ...
    if ($imap_emails) {
        rsort($imap_emails);
        /* For every email. Get csv Attachments. */
        foreach($imap_emails as $email_number) {
            $overview = current(imap_fetch_overview($imap, $email_number));
            if ($config->isDebug()) {
                echo ' - [' . $email_number . '] ' . $overview->subject . " \n";
            }
            if (preg_match('/noreply@smartsparrow.com/i', $overview->from) && $overview->answered == false && $overview->flagged == false && 
                    preg_match('/Student (.+) (.+) has finished Workshop Testing/i', $overview->subject, $regs)) {
